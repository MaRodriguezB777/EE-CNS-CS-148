...
output_folder = f'./seed={seed}_correct_function_output/'
input_size = 100
...
def test_mlp_backward():
    ...
    assert np.allclose(outputs['yhat'], corr_outputs['yhat']), f'backward failed -- yhat outputs are not close enough'

    try:
        w_bool_optA = True
        b_bool_optA = True
        for i in range(len(outputs['gradients'])):
            w_bool_optA = w_bool_optA and np.allclose(outputs['gradients'][i]['dW'], corr_outputs['gradients'][i]['dW'])
            b_bool_optA = b_bool_optA and np.allclose(outputs['gradients'][i]['db'], corr_outputs['gradients'][i]['db'])
    except ValueError:
        # print('Failed when testing reverse order for gradients')
        w_bool_optA = False
        b_bool_optA = False
    ...
    try:
        ...
        b_bool_optB = True
        for i in range(len(outputs['gradients'])):
            w_bool_optB = w_bool_optB and np.allclose(outputs['gradients'][-(i + 1)]['dW'], corr_outputs['gradients'][i]['dW'])
            b_bool_optB = b_bool_optB and np.allclose(outputs['gradients'][-(i + 1)]['db'], corr_outputs['gradients'][i]['db'])
    except ValueError:
        # print('Failed when testing forward order for gradients')
        w_bool_optB = False
        b_bool_optB = False

    assert (w_bool_optA or w_bool_optB),  f'backward failed -- gradient W outputs are not close enough in either order'
    assert (b_bool_optA or b_bool_optB),  f'backward failed -- gradient b outputs are not close enough in either order'
